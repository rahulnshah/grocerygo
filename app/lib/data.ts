...
export async function fetchCheckedItemsById(id: number) {
    noStore();
    try {
        const data = await sql<CheckedItemsCount>`
      SELECT 
    ...
    `;
        ...
        return data.rows[0]?.checked_items_count || 0;;
    } catch (error) {
        console.error('Database Error:', error);
    }
}

export async function fetchTotalItemsById(id: number) {
    noStore();
    try {
        const data = await sql<ItemsCount>`
      SELECT 
    ...
    `;
        ...
        return data.rows[0]?.items_count || 0;;
    } catch (error) {
        console.error('Database Error:', error);
    }
}
